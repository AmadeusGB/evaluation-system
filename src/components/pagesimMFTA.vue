...
export default {
  ...
  created: async function() {
    ...
    for (var i = 0; i < num; i++) {
      var tmp = await Demo.displaymodelinfo(temp[i], 2,1);
      if(tmp == 0) missionname = "M1";
      else if(tmp == 1) missionname = "M2";
      else if(tmp == 2) missionname = "M3";
      
      var tmp = await Demo.displaymodelinfo(temp[i], 6,1);
      if(tmp == 0) evaluationstate = "评估中";
      else evaluationstate = "评估任务完成";

      str =
        '{"ordernumber":"' + 
        (await Demo.displaymodelinfo(temp[i], 1,1))    +
        '","name":"'     +  missionname                +
        '","createtime":"'       +
        (await Demo.displaymodelinfo(temp[i], 3,1))    +
        '","evaluationaddress":"'    +
        (await Demo.displayaddressinfo(temp[i], 4,1)) +
        '","servicetoken":"'   + 
        (await Demo.displaymodelinfo(temp[i], 5,1))    +
        '","timecost":"'    +
        (await Demo.displaymodelinfo(temp[i], 6,1))    +
        '","load":"'   + 
        (await Demo.displaymodelinfo(temp[i], 7,1))    +
